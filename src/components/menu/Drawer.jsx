  ...
  return (
    <div className="text-white shadow-xl">
      ...
      <div className="w-[100%]  bg-white text-black   py-4 px-2 flex  justify-between items-center m-auto">
        <Link
          to={ALL_LINKS.HomePage.pageLink}
          className="text-4xl font-semibold tracking-wide hover:scale-105 duration-200 hover:text-purple-500"
          id="RubyVinyl"
        >
          ANIMART
        </Link>
        <div className="flex items-center space-x-4 smrev:space-x-2">
          ...
          {['left'].map((anchor) => (
            <React.Fragment key={anchor}>
              <div className={`hover:cursor-pointer bg-black rounded-full p-2 ${authCtx.role === 'seller' ? '' : ''}`} onClick={toggleDrawer(anchor, true)}>
