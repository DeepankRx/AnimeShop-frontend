...
const Collections = () => {
  ...
  const Card = ({ className, title, image, first }) => {
    return (
      <div
        className={`h-[600px]  bg-red-200 ${className} relative mdrev:col-span-6 mdrev:h-[400px] lgrev:h-[500px] cursor-pointer `}
        onClick={() => navigate(`${ALL_LINKS.Category.pageLink}?search=${title}`)}>
        ...
        <div className="absolute top-0 w-[100%] h-[100%] backdrop-brightness-[.60] hover:backdrop-brightness-[.50]  text-white flex justify-center items-center flex-col gap-4">
          {first ? <h1 className="text-4xl font-bold uppercase px-8">{title}</h1> : <h2 className="text-2xl font-semibold uppercase px-8">{title}</h2>}
  ...
  return (
    <div className="p-8  grid grid-cols-6 gap-8 mdrev:px-4 mdrev:py-8">
      <Card className="col-span-2" title="T-SHIRT" image={assets.collections.shirt} first />
