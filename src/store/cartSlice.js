...
export const cartSlice = createSlice({
  ...
  reducers: {
    addItemToCart: (state, action) => {
      ...
      const updatedAmount = state.totalAmount + data.item.price * data.item.amount;
      const existingCartItemIndex = state.items.findIndex((item) => item._id === data.item._id && item.size === data.item.size);
    ...
    removeItemFromCart: (state, action) => {
      const data = action.payload;
      const existingCartItemIndex = state.items.findIndex((item) => item._id === data._id && item.size === data.size);
      ...
      if (existingCartItem.amount === 1 || existingCartItem.amount === data.amount) {
        ...
        state.items.map((item, index) => {
          if (index !== existingCartItemIndex) updatedItems.push(item);
        });
      } else {
        const updatedItem = {
          ...existingCartItem,
