      ...
      <div>
        ...
        >
          <div className="rounded-lg flex flex-col">
            <Button onClick={() => navigate(`/dashboard/${edit_link}?productId=${productId}&edit=true`)} size="medium">
              Edit
            </Button>
            <Button
              size="medium"
    ...
    return (
      <div className="col-span-1 bg-white  rounded-xl overflow-hidden relative border-2 border-black">
        <div className="w-[100%] max-h-[200px] flex justify-center items-center overflow-hidden relative">
          <img src={product.images[0]} className="w-[100%]  object-cover" alt="product" />
        ...
        <div className="p-2 rounded-xl">
          ...
          {product.variants.map((item, i) => (
            <h2 key={i} className=" text-sm">
              {item.size} - <span className="font-bold text-lg">{item.countInStock}</span>
            </h2>
          ))}
          ...
          <div className="mr-4  smrev:mr-0 flex flex-col gap-4">
            ...
            <div className="flex justify-between smrev:flex-col gap-2">
              ...
              <div>
                <div className="p-2 bg-blue-100 rounded-md shadow-xl flex items-center">
                  ...
                  <input
                    placeholder="Search"
                    className="appearance-none bg-blue-100 text-sm  w-[100%] !outline-none  "
                    type="text"
                    name="search"
                    value={search}
                    onChange={(e) => {
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-4 sm:grid-cols-2">
              {loading ? (
                ...
                filteredProducts.length > 0 && filteredProducts.map((product, i) => <ProductCard key={i} product={product} />)
              )}
            </div>
