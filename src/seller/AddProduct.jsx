  ...
  const onSubmit = (values, { resetForm }) => {
    if (preview.length === 0) {
      ...
      if (editingMode) {
        ...
        updateProduct(queryParams.get('productId'), formData)
          .then(() => {
  ...
  const validationSchema = Yup.object({
    ...
    variants: Yup.array().of(
      Yup.object({
        ...
        countInStock: Yup.number().required('Required').min(0, 'Min value should be 0')
      })
    ),
        ...
        <div className="mr-4 p-4 smrev:mr-0">
          <Formik initialValues={fetchedValues || initialValues} validationSchema={validationSchema} onSubmit={onSubmit} enableReinitialize>
            <Form className="flex flex-col gap-4">
                      ...
                      <>
                        {subCategories.map((item, index) => (
                          <div className="relative col-span-1" key={index}>
                            ...
                            {index > 0 && (
                              <IconButton onClick={() => remove(index)} sx={{ position: 'absolute', top: 8, right: -12 }} aria-label="delete" size="small">
                                <CancelIcon />
                              </IconButton>
                            )}
                          </div>
                        ...
                        {variants.map((item, index) => (
                          <div className="relative col-span-2  grid grid-cols-2 lgrev:grid-cols-1  gap-4" key={index}>
                            ...
                            <InputField
                              ...
                              min={0}
                              uni={`variants.${index}.countInStock`}
                              placeholder="99"
                            />
                            {index > 0 && (
                              <IconButton onClick={() => remove(index)} sx={{ position: 'absolute', top: 8, right: -12 }} aria-label="delete" size="small">
              ...
              <a href={ALL_LINKS.SizingGuide.pageLink} target="_blank" className="text-blue-500 cursor-pointer" rel="noreferrer">
                Check Size Chart
              </a>
                        ...
                        {descriptions.map((item, index) => (
                          <div className="col-span-3 relative" key={index}>
                            <InputField
                              ...
                              uni={`descriptions.${index}`}
                              placeholder="Branded Shirt from Zerox."
                            />
                            {index > 0 && (
                              <IconButton onClick={() => remove(index)} sx={{ position: 'absolute', top: 8, right: -12 }} aria-label="delete" size="small">
                                <CancelIcon />
                  ...
                  preview.map((item, i) => (
                    <div className="relative" key={i}>
                      <div key={i} className=" w-full h-60 rounded-md flex justify-center items-center overflow-hidden">
                        <img className="object-contain w-[100%] h-[100%]" src={item} alt="preview" />
                      </div>
                      <IconButton
                        onClick={() => {
                          setPreview(
                  ...
                  <div className="border-2 border-dotted w-full h-60 border-black rounded-md flex justify-center items-center lgrev:my-2">
                    <div className=" right-0 bottom-8">
                      <label htmlFor="profile_pic">
                        <FontAwesomeIcon className="text-2xl text-tertiary" icon={faAdd}></FontAwesomeIcon>
                      </label>
                      <input id="profile_pic" hidden type="file" onChange={(e) => addImages(e)} accept="image/png, image/jpeg"></input>
                    </div>
                  </div>
                )}
              </FormWrapper>
